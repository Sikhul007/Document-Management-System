...
namespace DMS.Repository.Document
{
    public class DocumentRepository : IDocumentRepository
    {
        ...
        public object GetMyDocumentDetails(string userName, int page, int pageSize, string searchTerm, string sortColumn, string sortDirection)
        {
            var list = new List<DocumentModel>();
            int totalRecords = 0;
            ...
            using (var conn = new SqlConnection(_connectionString))
            {
                ...
                string countQuery = "SELECT COUNT(*) FROM Documents WHERE CreatedBy = @CreatedBy";

                if (!string.IsNullOrEmpty(searchTerm))
                {
                    countQuery += " AND (Title LIKE @SearchTerm OR Description LIKE @SearchTerm)";
                }

                using (var countCmd = new SqlCommand(countQuery, conn))
                {
                    countCmd.Parameters.AddWithValue("@CreatedBy", userName);
                ...
                // Build main query
                string sql = "SELECT Id, Title, Description FROM Documents WHERE CreatedBy = @CreatedBy";
                ...
                if (!string.IsNullOrEmpty(sortColumn))
                {
                    ...
                    var validColumns = new[] { "Title", "Description" };
                    ...
                    {
                        sql += " ORDER BY CreatedOn DESC"; // Default
                    }
                }
                else
                {
        ...
        public object GetPendingDocumentDetailsWithHeader(int page, int pageSize, string searchTerm, string sortColumn, string sortDirection, string userName = null)
        {
            var list = new List<DocumentDetailsModel>();
            int totalRecords = 0;
            ...
            using (var conn = new SqlConnection(_connectionString))
            {
                ...
                {
                    countQuery += " AND d.CreatedBy = @UserName";
                }

                if (!string.IsNullOrEmpty(searchTerm))
                {
                    countQuery += @" AND (d.Title LIKE @SearchTerm 
                               OR d.Description LIKE @SearchTerm 
                               OR d.CreatedBy LIKE @SearchTerm 
                               OR dd.OriginalFileName LIKE @SearchTerm)";
                }
                ...
                string sql = @"SELECT d.Id AS DocumentId, d.Title,
                        ...
                        dd.Id, dd.OriginalFileName, dd.VersionNumber, 
                        dd.ApproveStatus, dd.Description
                        FROM DocumentDetails dd
                        INNER JOIN Documents d ON dd.DocumentId = d.Id
                        WHERE dd.ApproveStatus = 'pending'";
                ...
                if (!string.IsNullOrEmpty(searchTerm))
                {
                    sql += @" AND (d.Title LIKE @SearchTerm 
                        OR d.Description LIKE @SearchTerm 
                        OR d.CreatedBy LIKE @SearchTerm 
                        OR dd.OriginalFileName LIKE @SearchTerm)";
                }
            ...
            {
                { "Title", "d.Title" },
                { "FileUploadedTime", "dd.CreatedOn" },
                { "Description", "d.Description" },
                { "CreatedBy", "d.CreatedBy" },
                { "DocumentCreatedOn", "d.CreatedOn" },
                { "OriginalFileName", "dd.OriginalFileName" },
                { "VersionNumber", "dd.VersionNumber" }
            };

                    if (columnMapping.ContainsKey(sortColumn))
                    {
                        sql += $" ORDER BY {columnMapping[sortColumn]} {direction}";
                    }
                    else
                    {
                        sql += " ORDER BY d.CreatedOn DESC"; // Default sorting
                    }
                }
                else
                {
        ...
        public object GetAllDocuments(int page, int pageSize, string searchTerm, string sortColumn, string sortDirection)
        {
            var list = new List<DocumentModel>();
            int totalRecords = 0;
            ...
            using (var conn = new SqlConnection(_connectionString))
            {
                ...
                // Get total count with search filter
                string countQuery = "SELECT COUNT(*) FROM Documents WHERE 1=1";

                if (!string.IsNullOrEmpty(searchTerm))
                {
                    countQuery += " AND (Title LIKE @SearchTerm OR Description LIKE @SearchTerm OR CreatedBy LIKE @SearchTerm)";
                }
                ...
                if (!string.IsNullOrEmpty(searchTerm))
                {
                    sql += " AND (Title LIKE @SearchTerm OR Description LIKE @SearchTerm OR CreatedBy LIKE @SearchTerm)";
                }
                ...
                if (!string.IsNullOrEmpty(sortColumn))
                {
                    ...
                    // Validate column to prevent SQL injection
                    var validColumns = new[] { "Title", "UploadedDate", "Description", "CreatedBy" };
                    if (validColumns.Contains(sortColumn))
                    {
                        sql += $" ORDER BY {sortColumn} {direction}";
                    }
        ...
        public void DeleteDocumentDetailById(int documentDetailId)
        {
            using (var conn = new SqlConnection(_connectionString))
            {
                ...
                using (var tran = conn.BeginTransaction())
                {
                    try
                    {
                        // 0. Optional: check and log dependent rows
                        string sqlCheck = "SELECT COUNT(1) FROM Notifications WHERE DocumentDetailId = @Id";
                        using (var cmdCheck = new SqlCommand(sqlCheck, conn, tran))
                        {
                            cmdCheck.Parameters.AddWithValue("@Id", documentDetailId);
                            var count = (int)cmdCheck.ExecuteScalar();
                            // you can use count for logging or business rule decisions
                        }

                        // 1. Delete related notifications (dependent child)
                        string sqlNotifications = "DELETE FROM Notifications WHERE DocumentDetailId = @Id";
                        using (var cmdNotif = new SqlCommand(sqlNotifications, conn, tran))
                        {
                            cmdNotif.Parameters.AddWithValue("@Id", documentDetailId);
                            cmdNotif.ExecuteNonQuery();
                        }

                        // 2. Delete related tags
                        string sqlTags = "DELETE FROM DocumentTags WHERE DocumentDetailsId = @Id";
                        using (var cmdTags = new SqlCommand(sqlTags, conn, tran))
                        {
                            cmdTags.Parameters.AddWithValue("@Id", documentDetailId);
                            cmdTags.ExecuteNonQuery();
                        }

                        // 3. Delete the document detail
                        string sqlDetail = "DELETE FROM DocumentDetails WHERE Id = @Id";
                        using (var cmdDetail = new SqlCommand(sqlDetail, conn, tran))
                        {
        ...
        public object GetDetailsWithNotes(string createdBy, int page, int pageSize, string searchTerm, string sortColumn, string sortDirection)
        {
            var result = new List<DocumentDetailsModel>();
            int totalRecords = 0;
            ...
            WHERE d.CreatedBy = @CreatedBy AND d.ApproveStatus = 'rejected'";
                ...
                if (!string.IsNullOrEmpty(searchTerm))
                {
                    countQuery += " AND (d.Description LIKE @SearchTerm OR h.Notes LIKE @SearchTerm)";
                }

                using (var countCmd = new SqlCommand(countQuery, conn))
                {
                    countCmd.Parameters.AddWithValue("@CreatedBy", createdBy ?? (object)DBNull.Value);
            ...
            WHERE d.CreatedBy = @CreatedBy AND d.ApproveStatus = 'rejected'";
                ...
                if (!string.IsNullOrEmpty(searchTerm))
                {
                    query += " AND (d.Description LIKE @SearchTerm OR h.Notes LIKE @SearchTerm)";
                }
                    ...
                    var columnMapping = new Dictionary<string, string>
            {
                ...
                { "Notes", "h.Notes" }
                    ...
                    if (columnMapping.ContainsKey(sortColumn))
                    {
                        query += $" ORDER BY {columnMapping[sortColumn]} {direction}";
                    }
                    else
                    {
                        query += " ORDER BY d.CreatedOn DESC"; // Default
                    }
                }
                else
                {
