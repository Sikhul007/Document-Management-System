...
public class DocumentService : IDocumentService
{
    ...
    {
        return _documentRepository.GetMyDocumentDetails(userName, page, pageSize, searchTerm, sortColumn, sortDirection);
    }
    ...
    {
        return _documentRepository.GetPendingDocumentDetailsWithHeader(page, pageSize, searchTerm, sortColumn, sortDirection, userName);
    }
    ...
    {
        return _documentRepository.GetAllDocuments(page, pageSize, searchTerm, sortColumn, sortDirection);
    }
    ...
    {
        return _documentRepository.GetDetailsWithNotes(createdBy, page, pageSize, searchTerm, sortColumn, sortDirection);
    }
    ...
    public void AddNewDocumentVersionToDocument(int documentId,
    DocumentModel model,
    List<IFormFile> files,
    List<string> fileDescriptions,
    string createdBy,
    ...
    List<List<int>> TagIds)
    {
        ...
        using (SqlConnection conn = new SqlConnection(_connectionString))
        {
            ...
            using (SqlTransaction transaction = conn.BeginTransaction())
            {
                try
                {
                    ...
                    int nextVersion = 1;
                    string getMaxVersionSql = "SELECT ISNULL(MAX(VersionNumber), 0) FROM DocumentDetails WHERE DocumentId = @DocumentId";
                    using (var cmd = new SqlCommand(getMaxVersionSql, conn, transaction))
                    {
                    ...
                    for (int i = 0; i < files.Count; i++)
                    {
                        ...
                        var details = new DocumentDetailsModel
                        {
                            ...
                            ApproveStatus = status,
                            IsArchive = isArchive,
                            ParentDocumentId = 0, // No parent since it's a new version line
                            LastUpdateBy = createdBy,
                            LastUpdateOn = DateTime.Now
                        };
                        int documentDetailId = _documentRepository.InsertDocumentDetails(details, conn, transaction);

                        uploadedNewVersions.Add((documentDetailId, originalFileName, nextVersion));
        ...
        // Create notifications (same as UploadNewVersionMultiple)
        try
        {
            if (status == "pending" || status == "approved")
            {
